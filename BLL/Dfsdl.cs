...
namespace ZWL.BLL
{
    public class Dfsdl
    {
        ...
        public bool Exists(int nID)
        {
            ...
            strSql.Append("select count(1) from dbo.dfsdl");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            string strinsert = "INSERT INTO [dbo].[dfsdl] ([mobile], [content], [deadtime], [status], [eid]" +
                ", [userid], [password], [userport]) VALUES (@mobile, @content, @deadtime, @" +
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update dbo.dfsdl set ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete dbo.dfsdl ");
            strSql.Append(" where id=" + nID + " ");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,6)				};
            parameters[0].Value = nID;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            string strselect = "SELECT id, mobile, content, deadtime, status, eid, userid, password, userport FROM dbo.dfsdl ";
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT id, mobile, content, deadtime, status, eid, userid, password, userport FROM dbo.dfsdl ");
            if (strWhere.Trim() != "")
            {
