...
namespace ZWL.BLL
{
	...
	/// </summary>
	[Serializable]
    public partial class ERPXMJBXX_TEMP
    {
        public ERPXMJBXX_TEMP()
        { }
        #region Model
        private int _id;
        ...
        /// 得到一个对象实体
        /// </summary>
        public ERPXMJBXX_TEMP(int ID)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * ");
            strSql.Append(" FROM [ERPXMJBXX_TEMP] ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from [ERPXMJBXX_TEMP]");
            strSql.Append(" where ID=@ID ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into [ERPXMJBXX_TEMP] (");
            strSql.Append("XMBH,XMName,DWMC,JFHTJSJE,YSJE,YSJEBase,YFJE,YFJEBase,YSYFBaseDate,XMFZR,YWLXR,QKDWXZ,SFWG,GWSJ,LYQK,SFYJF,SFYJWHZ,CSCS,YSGCK,YSGCKBase,FBJSJE,YFLWF,YFLWFBase,HTBH,PZR,ZL,ZQZSQ,ZQFSSJ,YFDW,BM,DJR)");
            ...
            parameters[24].Value = PZR;
            parameters[25].Value = ZL;
            parameters[26].Value = ZQZSQ;
            parameters[27].Value = ZQFSSJ;
            parameters[28].Value = YFDW;
            parameters[29].Value = BM;
            parameters[30].Value = DJR;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [ERPXMJBXX_TEMP] set ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from [ERPXMJBXX_TEMP] ");
            strSql.Append(" where ID=@ID ");
            SqlParameter[] parameters = {
                    new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = ID;
        ...
        public ZWL.BLL.ERPXMJBXX_TEMP GetModelBySqlWhere(string strWhere)
        {
            var ds = GetList(strWhere);
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                return DataTableHelper.CreateItem<ZWL.BLL.ERPXMJBXX_TEMP>(ds.Tables[0].Rows[0]);
            }
            return null;
        }

        public List<ZWL.BLL.ERPXMJBXX_TEMP> GetModelList(string strWhere)
        {
            var result = new List<ZWL.BLL.ERPXMJBXX_TEMP>();
            var ds = GetList(strWhere);
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                result = DataTableHelper.ConvertTo<ZWL.BLL.ERPXMJBXX_TEMP>(ds.Tables[0]);
            }
            return result;
        }
        #endregion  Method
