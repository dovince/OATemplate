...
namespace ZWL.BLL
{
	...
	public class ERPNWorkFlow
	{
        ...
        public List<ZWL.BLL.ERPNWorkFlow> GetModelList(string strWhere)
        {
            var ds = GetList(strWhere);
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                var dt = ds.Tables[0];
                return DataTableHelper.ConvertTo<ZWL.BLL.ERPNWorkFlow>(dt);
            }
            return new List<ZWL.BLL.ERPNWorkFlow>();
        }
    }
}
