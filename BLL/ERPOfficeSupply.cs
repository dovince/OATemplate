...
namespace ZWL.BLL
{
	...
	/// </summary>
	[Serializable]
    public partial class ERPOfficeSupply
    {
        public ERPOfficeSupply()
        { }
        #region Model
        private int _id;
        private string _no;
        ...
        private List<ZWL.BLL.ERPOfficeSupplyDetail> _subItems = null;
        public List<ZWL.BLL.ERPOfficeSupplyDetail> SubItems
        {
            get
            {
                ...
                if (this.ID > 0)
                {
                    var _currentModel = new ZWL.BLL.ERPOfficeSupplyDetail();
                    _subItems = _currentModel.GetListBySqlWhere("SupplyID=" + this.ID);
                }
                return _subItems;
            }
            set
        ...
        /// 得到一个对象实体
        /// </summary>
        public ERPOfficeSupply(int ID)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * ");
            strSql.Append(" FROM [ERPOfficeSupply] ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from [ERPOfficeSupply]");
            strSql.Append(" where ID=@ID ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into [ERPOfficeSupply] (");
            strSql.Append("No,WorkName,CreatedDate,Department,Operator,NWorkID,State,Comment,ReservedField1,ReservedField2,ReservedField3,Total,Form)");
            ...
            SqlParameter[] parameters = {
                    ...
                    new SqlParameter("@Form", SqlDbType.Int,4)};
            ...
            parameters[11].Value = Total;
            parameters[12].Value = Form;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [ERPOfficeSupply] set ");
            strSql.Append("No=@No,");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from [ERPOfficeSupply] ");
            strSql.Append(" where ID=@ID ");
            SqlParameter[] parameters = {
                    new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = ID;
        ...
        private void SetPropertyValue(DataSet ds)
        {
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["Form"] != null && ds.Tables[0].Rows[0]["Form"].ToString() != "")
                {
                    this.Form = int.Parse(ds.Tables[0].Rows[0]["Form"].ToString());
                }
            }
        }

        public ZWL.BLL.ERPOfficeSupply GetModelBySqlWhere(string strWhere)
        {
            var ds = GetList(strWhere);
            if (ds != null && ds.Tables.Count > 0)
            {
                return DataTableHelper.CreateItem<ZWL.BLL.ERPOfficeSupply>(ds.Tables[0].Rows[0]);
            }
            return null;
        }
        ...
        public ZWL.BLL.ERPOfficeSupply GetModelByWhere(string strWhere)
        {
        ...
        public ZWL.BLL.ERPOfficeSupply GetModelByWorkId(int workid)
        {
            return GetModelByWhere("NWorkID=" + workid);
        }
        public List<ZWL.BLL.ERPOfficeSupply> GetListModel(string strWhere)
        {
            var list = new List<ZWL.BLL.ERPOfficeSupply>();
            var ds = GetList(strWhere);
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                list = DataTableHelper.ConvertTo<ZWL.BLL.ERPOfficeSupply>(ds.Tables[0]);
            }
            return list;
        }
        ...
        public Pager GetPagingList(string strWhere, int cPage, int pSize)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * ");
            strSql.Append(@" FROM (select a.*,t.JieDianName,t.ShenPiUserList,t.OKUserList,t.StateNow,t.WorkFlowID,t.FormID FROM [ERPOfficeSupply] a join [ERPNWorkToDo] t
  on a.NWorkID= t.ID) m ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
