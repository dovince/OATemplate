...
namespace ZWL.BLL
{
    ...
    public class ERPYinZhang
    {
        ...
        public List<ZWL.BLL.ERPYinZhang> GetListModel(string strWhere)
        {
            var result = new List<ZWL.BLL.ERPYinZhang>();
            var ds = GetList(strWhere);
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                var re = DataTableHelper.ConvertTo<ZWL.BLL.ERPYinZhang>(ds.Tables[0]);
                return re;
            }
            return result;
        }
