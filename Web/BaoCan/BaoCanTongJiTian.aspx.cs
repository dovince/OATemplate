...
public partial class HR_BaoCanTongJiTian : BasePage
{
    ...
    protected override void DataBindToGridview()
    {
        ...
        //if (!IsPostBack)
        //{
        //    timelimit += PublicMethod.GetSqlKeywordAnd(timelimit) + " (RecordDate >= '" + DateTime.Now.ToString("yyyy-MM-dd") + "') ";
        ...
        var sql = @"select isnull(d.RecordDate, e.BCRQ) RQ,* from (
                   select *,
                   (select count(0) c from FanTangJiuCanRecord d join [Flow] f on f.LotID=d.LotID where  {0} and d.RecordDate=t.RecordDate and CanShi='早餐' and [KaoQinRecord] != '-' and Operation=1
                    and f.LotID = (select top 1 f.LotID from FanTangJiuCanRecord d join [Flow] f on f.LotID=d.LotID and {0} and d.RecordDate=t.RecordDate and CanShi='早餐' and [KaoQinRecord] != '-' and Operation=1 order by createdTime desc)) ZaoCan,
                   (select count(0) c from FanTangJiuCanRecord d join [Flow] f on f.LotID=d.LotID where  {0} and d.RecordDate=t.RecordDate and CanShi='午餐' and [KaoQinRecord] != '-' and Operation=1 
                    and f.LotID = (select top 1 f.LotID from FanTangJiuCanRecord d join [Flow] f on f.LotID=d.LotID and {0} and d.RecordDate=t.RecordDate and CanShi='午餐' and [KaoQinRecord] != '-' and Operation=1 order by createdTime desc)) WuCan
                   from (
                   select RecordDate from FanTangJiuCanRecord where  {0} GROUP BY RecordDate
                   ) t
                   ) d
                   full join (
                   select *,
                   (select count(0) c from ERPBaoCan d where   {2} and d.BCRQ=t.BCRQ and ShiJianDian='早餐' and IsCancel='否' ) ZaoCan1,
                   (select count(0) c from ERPBaoCan d where   {2} and d.BCRQ=t.BCRQ and ShiJianDian='午餐' and IsCancel='否' ) WuCan1
                   from (
                   select BCRQ from ERPBaoCan where   {2} GROUP BY BCRQ
                   ) t
                   ) e
                   on d.RecordDate = e.BCRQ
                   where 1=1 {1} ".FormatWith(timelimit, sqlWhere, bcrqlimit);
        //var sql = SQLFormat.FormatWith(Get("LotID"), sqlWhere);
        //throw new Exception(sql);
        var pager = new Pager(sql, currPage, pageSize, " RQ asc ");
        if (pager != null)
        {
