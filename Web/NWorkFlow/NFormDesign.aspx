<%@ Page Language="C#" AutoEventWireup="true" CodeFile="NFormDesign.aspx.cs" Inherits="NWorkFlow_NFormDesign" ValidateRequest="false" %>
...
{
    var num=parseInt(100000 * Math.random());    
    var tempStrDefault=Trim(document.getElementById("Text60").value);
    if(tempStrDefault.length>0&&document.getElementById("TextBox110").value.length>0)
    {           
        var OptionStr=new Array();    
        OptionStr=tempStrDefault.split('|');
        var TempOptionStr="";
        ...
        for (i=0;i<OptionStr.length ;i++ )    
        {    
               TempOptionStr=TempOptionStr+"<label><input name=RadioButton"+cValue+" value="+OptionStr[i]+" type = "+optiontype+" id= "+OptionStr[i]+"_"+i+" >"+OptionStr[i]+"</label>";
        }
        editor.execCommand("insertHtml",'<div id="Radio'+cValue+'" method = "post" name="RadioList'+cValue+'" alt="'+document.getElementById("TextBox110").value+'" >   '+TempOptionStr+'  </div>');
       
        document.getElementById("TextBox17").value=document.getElementById("TextBox17").value+"|"+"Radio"+cValue+"_"+document.getElementById("TextBox110").value;//系统控件自动计数
    }
    else{alert('中文字段名称、下拉字段默认值不可为空！');}
}
else if(str=="1")
{
    if(document.getElementById("TextBox5").value.length>0){
        ...
        editor.execCommand("insertHtml", '<input id=Text' + cValue + '  name="Text' + cValue + '" alt="' + document.getElementById("TextBox5").value +'"  type="text"  style="border:0;width:100%;"/>');
...
else if(str=="2")
{
    if(document.getElementById("TextBox7").value.length>0){
        editor.execCommand("insertHtml", '<textarea id=TextArea' + cValue + ' name="TextArea' + cValue + '"  alt="' + document.getElementById("TextBox7").value +' " style="border: 0;width: 100%;"></textarea>');
...
else if(str=="12")
{
    if(document.getElementById("TextBox6").value.length>0){
        editor.execCommand("insertHtml", '<input id=Num' + cValue + '  name="Num' + cValue + '" alt="' + document.getElementById("TextBox6").value +'"    type="text"  style="IME-MODE: disabled;border:0;width:100%;"  onkeypress="var k=event.keyCode; return (k>=48&&k<=57)||k==46" onpaste="return !/\D/.test(clipboardData.getData(\'text\'))"  ondragenter="return false"/>');
...
else if(str=="13")
{
    ...
    if(document.getElementById("TextBox8").value.length>0){
        editor.execCommand("insertHtml", '<input id=Date' + cValue + ' name="Date' + cValue + '" alt="' + document.getElementById("TextBox8").value +'"    type="text"  onclick="setday(this)"   value=""  style="border:0;width:100%;"/>');
...
else if(str=="4")
{
    if(document.getElementById("TextBox1").value.length>0){
        editor.execCommand("insertHtml", '<input readonly id=Text' + cValue + ' name="Text' + cValue + '" alt="' + document.getElementById("TextBox1").value +'" type="text" value="宏控件-用户姓名" style="border:0;"/>');
...
else if(str=="5")
{
   ...
   editor.execCommand("insertHtml",'<input readonly id=Text'+cValue+' name="Text'+cValue+'" alt="'+document.getElementById("TextBox3").value+'" type="text" value="宏控件-用户部门" style="border:0;"/>');
...
else if(str=="6")
{
   ...
   editor.execCommand("insertHtml",'<input readonly id=Text'+cValue+'  name="Text'+cValue+'" alt="'+document.getElementById("TextBox4").value+'"  type="text" value="宏控件-用户角色" style="border:0;"/>');
...
else if(str=="7")
{
   ...
   editor.execCommand("insertHtml",'<input readonly id=Text'+cValue+'  name="Text'+cValue+'" alt="'+document.getElementById("TextBox10").value+'" type="text" value="宏控件-用户职位" style="border:0;"/>');
...
else if(str=="8")
{
   ...
   editor.execCommand("insertHtml",'<input readonly id=Text'+cValue+'  name="Text'+cValue+'" alt="'+document.getElementById("TextBox12").value+'" type="text" value="宏控件-当前日期" style="border:0;"/>');
...
else if(str=="101")
{
   ...
   editor.execCommand("insertHtml",'<input readonly id=Text'+cValue+'  name="Text'+cValue+'" alt="'+document.getElementById("TextBox13").value+'" onclick="selectUser(Text'+cValue+');"  type="text" value="点击选择用户" style="border:0;"/>');
...
else if(str=="102")
{
   ...
   editor.execCommand("insertHtml",'<input readonly id="Text'+cValue+'"  name="Text'+cValue+'" alt="'+document.getElementById("TextBox14").value+'" onclick="selectBuMen(Text'+cValue+');"  type="text" value="点击选择部门" style="border:0;"/>');
...
else if(str=="103")
{
   ...
   editor.execCommand("insertHtml",'<input readonly id=Text'+cValue+'  name="Text'+cValue+'"  alt="'+document.getElementById("TextBox15").value+'" type="text" value="宏控件-部门主管" style="border:0;"/>');
    ...
    <div>
        <table id="PrintHide" style="width: 100%" border="0" cellpadding="0" cellspacing="0">
            <tr>
                ...
                <td style="border-bottom: #006633 1px ; height: 30px; text-align: right" valign="middle">
        ...
        <table style="width: 100%" bgcolor="#999999" border="0" cellpadding="2" cellspacing="1">
            <tr>
                <td style="width: 208px; height: 25px; background-color: #f6f6f6; text-align: center;
                    vertical-align: top;" align="right">
                    <strong>权限设置列表<br />
                    </strong>&nbsp;<asp:TextBox ID="TextBox17" runat="server" Width="172px" Height="61px"></asp:TextBox>
                    <hr />
                    ...
                    <hr />
                    <hr />
                    <input id="Button130" onclick="formset(200)" style="width: 60px" type="button" value="单选框" />
                    中文名：<asp:TextBox ID="TextBox110" runat="server" Width="90px"></asp:TextBox><br />
                    &nbsp;默认：<input id="Text60" style="width: 162px" type="text" /><br />
                    <span style="color: #000033">*默认值用" | "分隔，例如：男|女</span><br />
                    <hr />
                    <strong>宏控件</strong>
                    <br />
                    <hr />
