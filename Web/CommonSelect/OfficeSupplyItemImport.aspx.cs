...
public partial class CommonSelect_OfficeSupplyItemImport : BasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            lblFormTitle.Text = "办公用品规格型号导入";
        }
    }
    ...
    private bool Validate(ref string msg)
    {
        ...
        else
        {
            ...
            var headers = new string[] { "序号", "分类", "物品名称", "规格型号", "单位", "单价", "备注" };
            Stream stream = null;
            try
            {
    ...
    private bool InsertToDB(DataTable dt)
    {
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            ...
            {
                var item = dt.Rows[i];
                var type = item["分类"].ToString();
                var stype = typeList.FirstOrDefault(x => x.Category == type);
                if (stype == null)
                {
                    if (type != "未分类")
                    {
                        var ctypeModel = new ZWL.BLL.ERPOfficeSupplyType()
                        {
                            Category = type,
                            Description = type,
                        ...
                        ctypeModel.ID = ctypeModel.Add();
                        typeList.Add(ctypeModel);
                        stype = ctypeModel;
                        AddLog(ctypeModel);
                    }
                    else
                    {
                        stype = typeList.FirstOrDefault(x => x.Category == "未分类");
                    }
                }
