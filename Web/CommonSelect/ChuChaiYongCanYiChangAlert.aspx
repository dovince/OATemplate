<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ChuChaiYongCanYiChangAlert.aspx.cs" Inherits="CommonSelect_YongCanYiChangAlert" %>
        ...
        $(document).ready(function () {
            $(".input_cxcalendar").each(function () {
                ...
                var a = new Calendar({
                    ...
                    var sdate = $("#SDate").val();
                    var edate = $("#EDate").val();
                    getYongCanYiChangInfoByDateChanged(sdate, edate);
                });
            });
            $("#ReceivedUser").on("blur", function () {
                ...
                var edate = $("#EDate").val();
                var users = $(this).val();
                getYongCanYiChangInfo(sdate, edate, users);
            });

        });
        function getYongCanYiChangInfoByDateChanged(sdate, edate) {
            getYongCanYiChangInfo(sdate, edate,'');
        }
        function getYongCanYiChangInfo(sdate, edate, users) {
            var option = {
                url: '../Services/Services.ashx',
                data: {
                    f: 'ChuChaiYongCanYiChangInfo',
                    sdate: sdate,
                    edate: edate,
                    users: users
                },
                ...
                success: function (data) {
                    if (data.Code) {
                        ...
                        var html = "<ol style='height:200px;overflow:scroll;'>";
                        for (var i = 0; i < data.Data.MList.length; i++) {
                            var item = data.Data.MList[i];
                            html += "<li style='list-style: decimal;'>" + item.Date + "，" + item.Name + "，" + item.Text + "</li>";
                        }
                        html += "</ol>";
    ...
    <form id="form1" runat="server">
        <div>
                                            ...
                                            <tr>
                                                <td>
                                                    <div class="lui_form_subject">
                                                        <asp:Label ID="lblFormTitle" runat="server" Text="出差期间领取生活补贴提醒"></asp:Label>
                                                    </div>
                                                </td>
                                            </tr>
                                        </tbody>
            ...
            <table class="tb_normal process_review_panel" width="100%">
                <tbody>
                    <tr>
                        <td class="td_normal_title" style="width: 15%;" title="所选日期不能大于当前日期">日期范围：</td>
                        <td colspan="2" style="text-align: left; height: 40px; padding-left: 5px;">
                            <div class="inputselectsgl" style="width: 21%; min-height: 20px;">
                                <div class="input">
                                    <asp:TextBox ID="SDate" runat="server" class="input_cxcalendar"></asp:TextBox>
                                </div>
                                <div class="inputdatetime" onclick="javascript:document.getElementById('SDate').click();"></div>
                            </div>
                            <div class="inputselectsgl" style="width: 21%; min-height: 20px;">
                                <div class="input">
                                    <asp:TextBox ID="EDate" runat="server" class="input_cxcalendar"></asp:TextBox>
                                </div>
                                <div class="inputdatetime" onclick="javascript:document.getElementById('EDate').click();"></div>
                            </div>
                        </td>
                    </tr>
                    <tr>
